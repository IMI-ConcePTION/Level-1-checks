---
title: 'CONCEPTION - Level 1 checks: Step 4 to 5 (OBSERVATION_PERIODS)'
output: 
  html_document:
    theme: spacelab
    toc: true
...
if ("OBSERVATION_PERIODS" %in% list.files(output_dir)){
  op_dir<-paste(output_dir, "OBSERVATION_PERIODS/", sep="")
  op_less<-paste(op_dir, "Masked/", sep="")
   dir.create(paste(op_dir,"tmp", sep=""))
tmp<-paste(op_dir, "tmp/", sep="")
} else {
#Create the OBSERVATION_PERIODS folder in the output dir
dir.create(paste(output_dir, "OBSERVATION_PERIODS", sep=""))
  op_dir<-paste(output_dir, "OBSERVATION_PERIODS/", sep="")
dir.create(paste(op_dir,"Masked", sep=""))
op_less<-paste(op_dir, "Masked/", sep="")
dir.create(paste(op_dir,"tmp", sep=""))
tmp<-paste(op_dir, "tmp/", sep="")
}
...
total_var<-as.data.table(cbind(table_name="OBSERVATION_PERIODS", 
...
actual_tables<-directory_CDM[grepl(pattern="OBSERVATION_PERIODS", x=directory_CDM)]
...
step4.3<-function(dt, var_name){
  ...
  #op_origin
  a.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, op_origin)]
  setnames(a.1, "op_origin", "vocabulary")
  a.1<-cbind(a.1, variable="op_origin")
  a.2<-dt[complete.cases(meaning) & complete.cases(op_origin), .N]
  a.2<-data.table(variable="op_origin", total=a.2)

  #op_meaning
  b.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, op_meaning)]
  setnames(b.1, "op_meaning", "vocabulary")
  b.1<-cbind(b.1, variable="op_meaning")
  b.2<-dt[complete.cases(meaning) & complete.cases(op_meaning), .N]
  b.2<-data.table(variable="op_meaning", total=b.2)
...
step4.4<-function(dt, var_name, date_var){
  ...
  #op_origin
  a.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, op_origin)]
  setnames(a.1, "op_origin", "vocabulary")
  a.1<-cbind(a.1, variable="op_origin")
  a.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(op_origin), .N, by=.(year)]
  setnames(a.2, "N", "total")
a.2<-data.table(variable="op_origin", a.2)
  
  #op_meaning
  b.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, op_meaning)]
  setnames(b.1, "op_meaning", "vocabulary")
  b.1<-cbind(b.1, variable="op_meaning")
  b.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(op_meaning), .N, by=.(year)]
  setnames(b.2, "N", "total")
  b.2<-data.table(variable="op_meaning", b.2)
...
step_4_5_check<- function(tables_list){
  ...
  for (i in 1:length(tables_list)){
    ...
    df[,year:=substr(op_start_date, 1, 4)]
    ...
    Res.4.3[[w]]<-step4.3(dt=df, var_name = "op_meaning")
    Res.4.4[[w]]<-step4.4(dt=df, var_name = "op_meaning")
    
        Res.p<-df[, c("person_id", "year", "op_meaning")][!is.na(person_id)][!is.na(op_meaning)]
      write.csv(Res.p, paste(tmp,"persons_", tables_list[i],sep="") ,row.names = F)
    rm(Res.p)
        
    Res.dates.m[[w]]<-dates_count.m(dt=df, var_name = "op_meaning", date_var1 = "op_start_date", date_var2 = "op_end_date")
    Res.dates.my[[w]]<-dates_count.my(dt=df, var_name = "op_meaning", date_var1 = "op_start_date", date_var2 = "op_end_date")
    
    Res.p.t[[w]]<-df[!is.na(op_meaning) & !is.na(person_id), .N]
    Res.p.t.y[[w]]<-df[!is.na(op_meaning) & !is.na(person_id),.(total=.N), by="year"][!is.na(year)]
    w<-w+1
    rm(df)
  }
  ...
  Res.p.c[, comb:=paste(person_id, op_meaning, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb:=paste(person_id, op_meaning, sep="_^_")][!duplicated(comb)]
    Res.p.c<-rbind(Res.p.c,a)
    Res.p.c<-Res.p.c[!duplicated(comb)]
    rm(a)
  }
  Res.p.c[,comb:=NULL]
    Res.p.c<-data.table(variable_name="person_id",Res.p.c[!is.na(op_meaning), .(count=.N), by="op_meaning"])
  ...
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "op_meaning")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "op_meaning")])][!duplicated(comb_y)]
    Res.p.c.y<-rbind(Res.p.c.y,a)
    Res.p.c.y<-Res.p.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.p.c.y[,comb_y:=NULL]
        Res.p.c.y<-cbind(variable_name="person_id", Res.p.c.y[!is.na(op_meaning), .(count=.N), by=.(op_meaning, year)])
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  setnames(Res.p.y, "op_meaning", "meaning")
...
i<-1
 ...
 Res.dates.my<-Res.dates.my[meaning !="NA"]
  ...
  output<-list("COUNT1_M"=Res.p, "COUNT1_MY"=Res.p.y, "COUNT2_M"=Res.4.3[vocabulary !="NA"], "COUNT2_MY"=Res.4.4[vocabulary !="NA"], "DATES.M"=Res.dates.m, "DATES_MY"=Res.dates.my, "DUP"=dup)
  return(output)
}
...
Results.op<-step_4_5_check(actual_tables)
```

```{r include=FALSE}
 unlink(paste0(op_dir, "tmp"), recursive = T)
```
...
Conventions:  
**1.** The OBSERVATION_PERIODS table does not contain any convention that can be checked by the script.    
...
The OBSERVATION_PERIODS table does not contain any convention that can be checked by the script.
...
Res.count2.m<-data.table(table_name="OBSERVATION_PERIODS", Results.op$COUNT2_M)
...
write.csv(Res.count2.m, paste(op_dir,"observation_periods_meaning_2categories.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.m, paste(op_less,"observation_periods_meaning_2categories_masked.csv",sep="") ,row.names = F)
```
...
Res.count1.m<-data.table(table_name="OBSERVATION_PERIODS", Results.op$COUNT1_M)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(op_dir,"observation_periods_meaning_other.csv",sep="") ,row.names = F)
...
Res.count1.m[, total:= as.character(total)][as.numeric(total) > 0 & as.numeric(total) < 5, total := "<5"] #replace total as well if lower than 5
...
#Save results as .csv file
write.csv(Res.count1.m, paste(op_less,"observation_periods_meaning_other_masked.csv",sep="") ,row.names = F)
```
...
Res.count2.my<-data.table(table_name="OBSERVATION_PERIODS", Results.op$COUNT2_MY)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(op_dir,"observation_periods_meaning_year_2categories.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(op_less,"observation_periods_meaning_year_2categories_masked.csv",sep=""),row.names = F)
```
...
Res.count1.my<-data.table(table_name="OBSERVATION_PERIODS", Results.op$COUNT1_MY)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(op_dir,"observation_periods_meaning_year_other.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(op_less,"observation_periods_meaning_year_other_masked.csv",sep=""), row.names = F)
```
...
The OBSERVATION_PERIODS table does not contain continous variables.
<br>
...
Res.dates.m<-data.table(table_name="OBSERVATION_PERIODS", Results.op$DATES.M)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(op_dir,"observation_periods_meaning_dates.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(op_less,"observation_periods_meaning_dates_masked.csv",sep=""), row.names = F)
```
...
Res.dates.my<-data.table(table_name="OBSERVATION_PERIODS",Results.op$DATES_MY)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(op_dir,"observation_periods_meaning_year_dates.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(op_less,"observation_periods_meaning_year_dates_masked.csv",sep=""),row.names = F)
```
...
Numerator= count  
Denominator= total   
Year= retrieved from the year part of the variable `op_start_date`. Exception for dates count stratified by meaning and year where year will be retrieved for each of the dates respectively. 

```{r calculations_table, echo=FALSE}
calculations_op<-data.table(rbind(
  cbind(calculation= "Counts stratified by meaning(other variables)", variable_name=c("person_id"), numerator=c("Number of unique persons"), denominator=c("Number of total observations with a recorded meaning")),
    cbind(calculation= "Counts stratified by meaning and year(other variables)", variable_name=c("person_id"), numerator=c("Number of unique persons"), denominator=c("Number of total observations with a recorded meaning in a particular year")),
      cbind(calculation= "Counts stratified by meaning(2 or more categories)", variable_name=c("op_origin", "op_meaning"), numerator=c("Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
      cbind(calculation= "Counts stratified by meaning and year(2 or more categories)", variable_name=c("op_origin", "op_meaning"), numerator=c("Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning", variable_name=c("op_start_date", "op_end_date"), numerator=c( "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning and year", variable_name=c("op_start_date", "op_end_date"), numerator=c( "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year"))
))
datatable(calculations_op)
```
...
OBSERVATION_PERIODS     
**1.** OBSERVATION_PERIODS.html: Rmarkdown report      
**2.** `observation_periods_meaning_2categories.csv`: counts of variables with 2 or more categories stratified by meaning      
**3.** `observation_periods_meaning_other.csv`: counts of other variables stratified by meaning          
**4.** `observation_periods_meaning_year_2categories.csv`: counts of variables with 2 or more categories stratified by meaning and year         
**5.** `observation_periods_meaning_year_other.csv`: counts of other variables stratified by meaning and year         
**6.** `observation_periods_meaning_dates.csv`: counts of date variables stratified by meaning           
**7.** `observation_periods_meaning_year_dates.csv`: counts of date variables stratified by meaning and year    
**8.** Masked:      
&nbsp;&nbsp;&nbsp;&nbsp;***a.*** `observation_periods_meaning_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning where counts smaller than 5 are masked          
&nbsp;&nbsp;&nbsp;&nbsp;***b.*** `observation_periods_meaning_other_masked.csv`: counts of other variables stratified by meaning where counts smaller than 5 are masked       
&nbsp;&nbsp;&nbsp;&nbsp;***c.*** `observation_periods_meaning_year_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning and year where counts smaller than 5 are masked            
&nbsp;&nbsp;&nbsp;&nbsp;***d.*** `observation_periods_meaning_year_other_masked.csv`: counts of other variables stratified by meaning and year where counts smaller than 5 are masked     
&nbsp;&nbsp;&nbsp;&nbsp;***e.*** `observation_periods_meaning_dates_masked.csv`: counts of date variables stratified by meaning where counts smaller than 5 are masked               
&nbsp;&nbsp;&nbsp;&nbsp;***f.*** `observation_periods_meaning_year_dates_masked.csv`: counts of date variables stratified by meaning and year where counts smaller than 5 are masked    
