---
title: 'CONCEPTION - Level 1 checks: Step 4 to 5 (EVENTS)'
output: 
  html_document:
    theme: spacelab
    toc: true
...
if ("EVENTS" %in% list.files(output_dir)){
 ...
 dir.create(paste(events_dir,"tmp", sep=""))
tmp<-paste(events_dir, "tmp/", sep="")
} else {
...
total_var<-as.data.table(cbind(table_name="EVENTS", 
        variable_name=c("person_id", "start_date_record", "end_date_record", "event_code",
                        ...
                        "present_on_admission","laterality_of_event", "meaning_of_event", "origin_of_event", "visit_occurrence_id")))
```
...
actual_tables<-directory_CDM[grepl(pattern="EVENTS", x=directory_CDM)]
...
#Check conventions
convention_check_events<-function(dt){
  error<-rep(NA, 3)
  comment<-rep(NA, 3)
  
  con.1<-dt[(is.na(event_code) & is.na(event_free_text)), .N]
  if (con.1!=0){
    error[1]<-TRUE
    comment[1]<-paste("Convention 1 is not satisfied.There is(are) ", dt[(is.na(event_code) & is.na(event_free_text)), .N], 
                      " observation(s) that have missing values in both event_code and event_free_text.")
    } else {
      error[1]<-FALSE
      comment[1]<-"Convention 1 is satisfied."
    }
  
  dt[is.na(event_code), filter_1:=0][!is.na(event_code), filter_1:=1]
  dt[is.na(event_free_text), filter_2:=0][!is.na(event_free_text), filter_2:=1]
  dt[filter_1==0 & filter_2==0, filter:=0][filter_1==1 & filter_2==1, filter:=1][filter_1==1 & filter_2==0, filter:=1][filter_1==0 & filter_2==1, filter:=1]
  dt[,c("filter_1", "filter_2"):=NULL]
  dt[is.na(event_record_vocabulary), filter_1:=0][!is.na(event_record_vocabulary), filter_1:=1]
  dt[filter_1==0 & filter==0, filter_f:=0][filter_1==1 & filter==1, filter_f:=0][filter_1==1 & filter==0, filter_f:=2][filter_1==0 & filter==1, filter_f:=1]
  #Meanings
  #filter_f==0 no error
  #filter_f==1 there are missing values in event_record_vocabulary
  #filter_f==2 there are values in event_record_vocabulary when neither event_code or event_free_text is complete
  
  con.2.1<-dt[filter_f==1, .N]
  con.2.2<-dt[filter_f==2, .N]
  
  if (con.2.1 >0){
    if (con.2.2 >0){
     error[2]<-TRUE
     comment[2]<-paste("Convention 2 is not satisfied.There is(are) ", con.2.1, 
                      " observation(s) that have missing data on event_record_vocabulary and there is(are) ", con.2.2, " observation(s) that have data on event_record_vocabulary when neither event_code or event_free_text is complete.")
    } else {
    error[2]<-TRUE
    comment[2]<-paste("Convention 2 is not satisfied.There is(are) ", con.2.1, 
                      " observation(s) that have missing data on event_record_vocabulary.")
  }
    } else {
    if (con.2.2 >0){
    error[2]<-TRUE
    comment[2]<-paste("Convention 2 is not satisfied.There is(are) ", con.2.2, " observation(s) that have data on event_record_vocabulary when neither event_code or event_free_text is complete.")
    } else {
      error[2]<-FALSE
  ...
  con.3<-dt[!grepl("^free_text", dt[["event_record_vocabulary"]]) & !is.na(event_free_text), .N]
  if(con.3>0){
    error[3]<-TRUE
    comment[3]<-paste("Convention 3 is not satisfied.There is(are) ", con.3, 
                      " observation(s) where event_record_vocabulary does not start with free_text when event_free_text is filled out.")
  } else {
    error[3]<-FALSE
    comment[3]<-"Convention 3 is satisfied."
  }
...
step4.1<-function(dt, var_name){
  ...
  #event_code
  c.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("event_code"), by=.(meaning)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning"), measure.vars=colnames(c.1)[-1]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(event_code), .N]
  c.2<-data.table(variable="event_code", total=c.2)
  
  #text_linked_to_event_code
  d.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("text_linked_to_event_code"), by=.(meaning)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning"), measure.vars=colnames(d.1)[-1]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(text_linked_to_event_code), .N]
  d.2<-data.table(variable="text_linked_to_event_code", total=d.2)
  
  #event_free_text
  e.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("event_free_text"), by=.(meaning)] 
  e.1<-data.table::melt(e.1, id.vars=c("meaning"), measure.vars=colnames(e.1)[-1]) #from wide to long  
  setnames(e.1, "value", "count")
  e.2<-dt[complete.cases(meaning) & complete.cases(event_free_text), .N]
  e.2<-data.table(variable="event_free_text", total=e.2)
...
step4.2<-function(dt, var_name){
  ...
  #event_code
  c.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("event_code"), by=.(meaning, year)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning", "year"), measure.vars=colnames(c.1)[-c(1:2)]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(event_code), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="event_code", c.2)

  #text_linked_to_event_code
  d.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("text_linked_to_event_code"), by=.(meaning, year)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning", "year"), measure.vars=colnames(d.1)[-c(1:2)]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(text_linked_to_event_code), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="text_linked_to_event_code", d.2)

  #event_free_text
  e.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("event_free_text"), by=.(meaning, year)] 
  e.1<-data.table::melt(e.1, id.vars=c("meaning", "year"), measure.vars=colnames(e.1)[-c(1:2)]) #from wide to long  
  setnames(e.1, "value", "count")
  e.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(event_free_text), .N, by=.(year)]
  setnames(e.2, "N", "total")
  e.2<-data.table(variable="event_free_text", e.2)
...
step4.3<-function(dt, var_name){
  ...
  a.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, event_record_vocabulary)]
  setnames(a.1, "event_record_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="event_record_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(event_record_vocabulary), .N]
  a.2<-data.table(variable="event_record_vocabulary", total=a.2)

  #Present_on_admission
  b.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, present_on_admission)]
  setnames(b.1, "present_on_admission", "vocabulary")
  b.1<-cbind(b.1, variable="present_on_admission")
  b.2<-dt[complete.cases(meaning) & complete.cases(present_on_admission), .N]
  b.2<-data.table(variable="present_on_admission", total=b.2)
  
  #meaning_of_event
  c.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, meaning_of_event)]
  setnames(c.1, "meaning_of_event", "vocabulary")
  c.1<-cbind(c.1, variable="meaning_of_event")
  c.2<-dt[complete.cases(meaning) & complete.cases(meaning_of_event), .N]
  c.2<-data.table(variable="meaning_of_event", total=c.2)
  
  #origin_of_event
  d.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, origin_of_event)]
  setnames(d.1, "origin_of_event", "vocabulary")
  d.1<-cbind(d.1, variable="origin_of_event")
  d.2<-dt[complete.cases(meaning) & complete.cases(origin_of_event), .N]
  d.2<-data.table(variable="origin_of_event", total=d.2)
  
  #laterality_of_event
  e.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, laterality_of_event)]
  setnames(e.1, "laterality_of_event", "vocabulary")
  e.1<-cbind(e.1, variable="laterality_of_event")
  e.2<-dt[complete.cases(meaning) & complete.cases(laterality_of_event), .N]
  e.2<-data.table(variable="laterality_of_event", total=e.2)
...
step4.4<-function(dt, var_name){
  ...
  #event_record_vocabulary
  a.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, event_record_vocabulary)]
  setnames(a.1, "event_record_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="event_record_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(event_record_vocabulary), .N, by=.(year)]
  setnames(a.2, "N", "total")
a.2<-data.table(variable="event_record_vocabulary", a.2)
  
  #Present_on_admission
  b.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, present_on_admission)]
  setnames(b.1, "present_on_admission", "vocabulary")
  b.1<-cbind(b.1, variable="present_on_admission")
  b.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(present_on_admission), .N, by=.(year)]
  setnames(b.2, "N", "total")
  b.2<-data.table(variable="present_on_admission", b.2)
  
  #meaning_of_event
  c.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, meaning_of_event)]
  setnames(c.1, "meaning_of_event", "vocabulary")
  c.1<-cbind(c.1, variable="meaning_of_event")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(meaning_of_event), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="meaning_of_event", c.2)
  
  #origin_of_event
  d.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, origin_of_event)]
  setnames(d.1, "origin_of_event", "vocabulary")
  d.1<-cbind(d.1, variable="origin_of_event")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(origin_of_event), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="origin_of_event", d.2)

  #laterality_of_event
  e.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, laterality_of_event)]
  setnames(e.1, "laterality_of_event", "vocabulary")
  e.1<-cbind(e.1, variable="laterality_of_event")
  e.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(laterality_of_event), .N, by=.(year)]
  setnames(e.2, "N", "total")
  e.2<-data.table(variable="laterality_of_event", e.2)
...
step_4_5_check<- function(tables_list){
  ...
  for (i in 1:length(tables_list)){
    ...
    df[,year:=substr(start_date_record, 1, 4)]
    ...
    Res.convention[[w]]<-data.table(table_name="EVENTS", table_directory=tables_list[i], convention_check_events(dt=df))
    Res.4.1[[w]]<-step4.1(dt=df, var_name = "meaning_of_event")
    Res.4.2[[w]]<-step4.2(dt=df, var_name = "meaning_of_event")
    Res.4.3[[w]]<-step4.3(dt=df, var_name = "meaning_of_event")
    Res.4.4[[w]]<-step4.4(dt=df, var_name = "meaning_of_event")
    
    Res.p<-df[!duplicated(person_id), c("person_id", "year", "meaning_of_event")][!is.na(person_id)][!is.na(meaning_of_event)]
      write.csv(Res.p, paste(tmp,"persons_", tables_list[i],sep="") ,row.names = F)
    rm(Res.p)
    
    Res.v<-df[!duplicated(visit_occurrence_id), c("visit_occurrence_id", "year", "meaning_of_event")][!is.na(visit_occurrence_id)][!is.na(meaning_of_event)]
    if (Res.v[,.N]>0){
      write.csv(Res.v, paste(tmp,"visits_", tables_list[i],sep="") ,row.names = F)
    rm(Res.v)
    }
    
    Res.dates.m[[w]]<-dates_count.m(dt=df, var_name = "meaning_of_event", date_var1 = "start_date_record", date_var2 = "end_date_record")
    Res.dates.my[[w]]<-dates_count.my(dt=df, var_name = "meaning_of_event", date_var1 = "start_date_record", date_var2 = "end_date_record")
 
    Res.p.t[[w]]<-df[!is.na(meaning_of_event) & !is.na(person_id), .N]
    Res.p.t.y[[w]]<-df[!is.na(meaning_of_event) & !is.na(person_id),.(total=.N), by="year"][!is.na(year)]
    Res.v.t[[w]]<-df[!is.na(meaning_of_event) & !is.na(visit_occurrence_id), .N]
    Res.v.t.y[[w]]<-df[!is.na(meaning_of_event) & !is.na(visit_occurrence_id),.(total=.N), by="year"][!is.na(year)]
    w<-w+1
    rm(df)
     }
  ...
  Res.p.c[, comb:=paste(person_id, meaning_of_event, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb:=paste(person_id, meaning_of_event, sep="_^_")][!duplicated(comb)]
    Res.p.c<-rbind(Res.p.c,a)
    Res.p.c<-Res.p.c[!duplicated(comb)]
    rm(a)
  }
  Res.p.c[,comb:=NULL]
    Res.p.c<-data.table(variable_name="person_id",Res.p.c[!is.na(meaning_of_event), .(count=.N), by="meaning_of_event"])
  ...
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_event")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_event")])][!duplicated(comb_y)]
    Res.p.c.y<-rbind(Res.p.c.y,a)
    Res.p.c.y<-Res.p.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.p.c.y[,comb_y:=NULL]
        Res.p.c.y<-cbind(variable_name="person_id", Res.p.c.y[!is.na(meaning_of_event), .(count=.N), by=.(meaning_of_event, year)])
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  setnames(Res.p.y, "meaning_of_event", "meaning")
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  Res.v.c[, comb:=paste(visit_occurrence_id, meaning_of_event, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[, comb:=paste(visit_occurrence_id, meaning_of_event, sep="_^_")][!duplicated(comb)]
    Res.v.c<-rbind(Res.v.c,a)
    Res.v.c<-Res.v.c[!duplicated(comb)]
    rm(a)
  }
  Res.v.c[,comb:=NULL]
    Res.v.c<-data.table(variable_name="visit_occurrence_id",Res.v.c[!is.na(meaning_of_event), .(count=.N), by="meaning_of_event"])
  ...
  Res.v.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_event")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[!is.na(year)][, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_event")])][!duplicated(comb_y)]
    Res.v.c.y<-rbind(Res.v.c.y,a)
    Res.v.c.y<-Res.v.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.v.c.y[,comb_y:=NULL]
  Res.v.c.y<-cbind(variable_name="visit_occurrence_id", Res.v.c.y[!is.na(meaning_of_event), .(count=.N), by=.(meaning_of_event, year)])
...
#Res.v.c.y, Res.v.t.y counts and total stratified by meaning and year: visit_occurrence_id
  ...
  setnames(Res.v.y, "meaning_of_event", "meaning")
...
Results.events<-step_4_5_check(actual_tables)
```

```{r include=FALSE}
 unlink(paste0(events_dir, "tmp"), recursive = T)
```

```{r include=FALSE}
rm(convention_check_events, combine_counts1.m, combine_counts1.my, combine_counts2.m, combine_counts2.my, dates_count.m, dates_count.my, step_4_5_check, step4.1, step4.2, step4.3, step4.4)
```
...
Conventions:  
**1.** Each record must contain either a code (in `event_code`) or a free text box (in `event_free_text`).   
**2.** `event_record_vocabulary` is not allowed to have empty values for completed `event_code` or `event_free_text`.     
**3.**When `event_code` is filled out, `event_record_vocabulary` needs to be filled out with the chosen coding system and when `event_free_text` is filled out then `event_record_vocabulary` should be set to "free_text_**original language of text**".     
...
```{r results_convention, echo=FALSE}
#Output for convention check
Res.convention<-Results.events$CONVENTION
...
**a.** event_record_vocabulary  
**b** present_on_admission  
**c.** meaning_of_event  
**d.** origin_of_event  
**e.** laterality_of_event    
...
Res.count2.m<-data.table(table_name="EVENTS", Results.events$COUNT2_M)
...
write.csv(Res.count2.m, paste(events_dir,"events_meaning_2categories.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.m, paste(events_less,"events_meaning_2categories_masked.csv",sep="") ,row.names = F)
```

```{r graph_count2_m, echo=FALSE, warning=FALSE}
if (Res.count2.m.1[,.N] != 0){
#Create data for graph(remove counts less than 5)
...
Res.count1.m<-data.table(table_name="EVENTS", Results.events$COUNT1_M)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(events_dir,"events_meaning_other.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(events_less,"events_meaning_other_masked.csv",sep="") ,row.names = F)
```
...
Res.count2.my<-data.table(table_name="EVENTS", Results.events$COUNT2_MY)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(events_dir,"events_meaning_year_2categories.csv",sep=""),row.names = F)
...
datatable(Res.count2.my, option=list(scrollX=TRUE)) %>% formatStyle("percentage",
  ...
  'year', 
  color = styleInterval(c(1994, as.numeric(substr(as.character(Sys.Date()), 1, 4))), c('red', 'black', 'red')),
  fontWeight = styleInterval(c(1994, as.numeric(substr(as.character(Sys.Date()), 1, 4))), c('bold', 'normal', 'bold'))
)
}

#Save results as .csv file
write.csv(Res.count2.my, paste(events_less,"events_meaning_year_2categories_masked.csv",sep=""),row.names = F)
```
...
Res.count1.my<-data.table(table_name="EVENTS", Results.events$COUNT1_MY)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(events_dir,"events_meaning_year_other.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(events_less,"events_meaning_year_other_masked.csv",sep=""), row.names = F)
```
...
The EVENTS table does not contain continous variables.
<br>
...
```{r count.dates_m, echo=FALSE}
#Output for dates count by meaning
Res.dates.m<-data.table(table_name="EVENTS", Results.events$DATES.M)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(events_dir,"events_meaning_dates.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(events_less,"events_meaning_dates_masked.csv",sep=""), row.names = F)
```
...
```{r count.dates_my, echo=FALSE}
#Output for dates count by meaning and year
Res.dates.my<-data.table(table_name="EVENTS",Results.events$DATES_MY)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(events_dir,"events_meaning_year_dates.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(events_less,"events_meaning_year_dates_masked.csv",sep=""),row.names = F)
```

If years in the future or before 1995 are present they will not be plotted in the graphs.   

```{r graph_count.dates_my, echo=FALSE}
...
Numerator= count  
Denominator= total   
Year= retrieved from the year part of the variable `start_date_record`.Exception for dates count stratified by meaning and year where year will be retrieved for each of the dates respectively. 

```{r calculations_table, echo=FALSE}
calculations_events<-data.table(rbind(
  cbind(calculation= "Counts stratified by meaning(other variables)", variable_name=c("person_id", "visit_occurrence_id", "event_code", "event_free_text", "text_linked_to_event_code"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category", "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
    cbind(calculation= "Counts stratified by meaning and year(other variables)", variable_name=c("person_id", "visit_occurrence_id", "event_code", "event_free_text", "text_linked_to_event_code"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
      cbind(calculation= "Counts stratified by meaning(2 or more categories)", variable_name=c("event_record_vocabulary", "present_on_admission", "meaning_of_event", "origin_of_event", "laterality_of_event"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category","Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning","Number of total observations with a recorded meaning")),
      cbind(calculation= "Counts stratified by meaning and year(2 or more categories)", variable_name=c("event_record_vocabulary", "present_on_admission", "meaning_of_event", "origin_of_event", "laterality_of_event"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category","Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year","Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning", variable_name=c("start_date_record", "end_date_record"), numerator=c( "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning and year", variable_name=c("start_date_record", "end_date_record"), numerator=c( "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year"))
))
datatable(calculations_events)
```
...
EVENTS     
**1.** EVENTS.html: Rmarkdown report      
**2.** `events_meaning_2categories.csv`: counts of variables with 2 or more categories stratified by meaning      
**3.** `events_meaning_other.csv`: counts of other variables stratified by meaning          
**4.** `events_meaning_year_2categories.csv`: counts of variables with 2 or more categories stratified by meaning and year         
**5.** `events_meaning_year_other.csv`: counts of other variables stratified by meaning and year         
**6.** `events_meaning_dates.csv`: counts of date variables stratified by meaning           
**7.** `events_meaning_year_dates.csv`: counts of date variables stratified by meaning and year    
**8.** Masked:      
&nbsp;&nbsp;&nbsp;&nbsp;***a.*** `events_meaning_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning where counts smaller than 5 are masked          
&nbsp;&nbsp;&nbsp;&nbsp;***b.*** `events_meaning_other_masked.csv`: counts of other variables stratified by meaning where counts smaller than 5 are masked       
&nbsp;&nbsp;&nbsp;&nbsp;***c.*** `events_meaning_year_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning and year where counts smaller than 5 are masked            
&nbsp;&nbsp;&nbsp;&nbsp;***d.*** `events_meaning_year_other_masked.csv`: counts of other variables stratified by meaning and year where counts smaller than 5 are masked     
&nbsp;&nbsp;&nbsp;&nbsp;***e.*** `events_meaning_dates_masked.csv`: counts of date variables stratified by meaning where counts smaller than 5 are masked               
&nbsp;&nbsp;&nbsp;&nbsp;***f.*** `events_meaning_year_dates_masked.csv`: counts of date variables stratified by meaning and year where counts smaller than 5 are masked    
