---
title: 'CONCEPTION - Level 1 checks: Step 4 to 5 (MEDICAL_OBSERVATIONS)'
output: 
  html_document:
    theme: spacelab
    toc: true
...
if ("MEDICAL_OBSERVATIONS" %in% list.files(output_dir)){
  mo_dir<-paste(output_dir, "MEDICAL_OBSERVATIONS/", sep="")
  mo_less<-paste(mo_dir, "Masked/", sep="")
  dir.create(paste(mo_dir,"tmp", sep=""))
tmp<-paste(mo_dir, "tmp/", sep="")
} else {
#Create the MEDICAL_OBSERVATIONS folder in the output dir
dir.create(paste(output_dir, "MEDICAL_OBSERVATIONS", sep=""))
  mo_dir<-paste(output_dir, "MEDICAL_OBSERVATIONS/", sep="")
  dir.create(paste(mo_dir,"Masked", sep=""))
mo_less<-paste(mo_dir, "Masked/", sep="")
  dir.create(paste(mo_dir,"tmp", sep=""))
tmp<-paste(mo_dir, "tmp/", sep="")
}
...
total_var<-as.data.table(cbind(table_name="MEDICAL_OBSERVATIONS", 
        variable_name=c("person_id", "mo_date", "mo_code", "mo_record_vocabulary",
...
#List of actual tables that match MEDICAL_OBSERVATIONS pattern
actual_tables<-directory_CDM[grepl(pattern="MEDICAL_OBSERVATIONS", x=directory_CDM)]
...
step4.1<-function(dt, var_name){
  ...
  #mo_code
  c.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_code"), by=.(meaning)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning"), measure.vars=colnames(c.1)[-1]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(mo_code), .N]
  c.2<-data.table(variable="mo_code", total=c.2)
  
  #mo_source_column
  d.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_source_column"), by=.(meaning)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning"), measure.vars=colnames(d.1)[-1]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(mo_source_column), .N]
  d.2<-data.table(variable="mo_source_column", total=d.2)
  
    #mo_source_value
  e.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_source_value"), by=.(meaning)] 
  e.1<-data.table::melt(e.1, id.vars=c("meaning"), measure.vars=colnames(e.1)[-1]) #from wide to long  
  setnames(e.1, "value", "count")
  e.2<-dt[complete.cases(meaning) & complete.cases(mo_source_value), .N]
  e.2<-data.table(variable="mo_source_value", total=e.2)
  
count<-rbind(c.1,d.1,e.1)
total<-rbind(c.2,d.2,e.2)
...
step4.2<-function(dt, var_name){
  ...
  #mo_code
  c.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_code"), by=.(meaning, year)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning", "year"), measure.vars=colnames(c.1)[-c(1:2)]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_code), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="mo_code", c.2)

  #mo_source_column
  d.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_source_column"), by=.(meaning, year)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning", "year"), measure.vars=colnames(d.1)[-c(1:2)]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_source_column), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="mo_source_column", d.2)

    #mo_source_value
  e.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("mo_source_value"), by=.(meaning, year)] 
  e.1<-data.table::melt(e.1, id.vars=c("meaning", "year"), measure.vars=colnames(e.1)[-c(1:2)]) #from wide to long  
  setnames(e.1, "value", "count")
  e.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_source_value), .N, by=.(year)]
  setnames(e.2, "N", "total")
  e.2<-data.table(variable="mo_source_value", e.2)
...
step4.3<-function(dt, var_name){
  ...
  a.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, mo_record_vocabulary)]
  setnames(a.1, "mo_record_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="mo_record_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(mo_record_vocabulary), .N]
  a.2<-data.table(variable="mo_record_vocabulary", total=a.2)
  
  #mo_unit
  b.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, mo_unit)]
  setnames(b.1, "mo_unit", "vocabulary")
  b.1<-cbind(b.1, variable="mo_unit")
  b.2<-dt[complete.cases(meaning) & complete.cases(mo_unit), .N]
  b.2<-data.table(variable="mo_unit", total=b.2)
  
  #mo_meaning
  c.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, mo_meaning)]
  setnames(c.1, "mo_meaning", "vocabulary")
  c.1<-cbind(c.1, variable="mo_meaning")
  c.2<-dt[complete.cases(meaning) & complete.cases(mo_meaning), .N]
  c.2<-data.table(variable="mo_meaning", total=c.2)
  
  
  #mo_origin
  d.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, mo_origin)]
  setnames(d.1, "mo_origin", "vocabulary")
  d.1<-cbind(d.1, variable="mo_origin")
  d.2<-dt[complete.cases(meaning) & complete.cases(mo_origin), .N]
  d.2<-data.table(variable="mo_origin", total=d.2)
  
    #mo_source_table
  e.1<-dt[complete.cases(meaning),.(count=.N), by=.(meaning, mo_source_table)] 
  setnames(e.1, "mo_source_table", "vocabulary")
  e.1<-cbind(e.1, variable="mo_source_table")
  e.2<-dt[complete.cases(meaning) & complete.cases(mo_source_table), .N]
  e.2<-data.table(variable="mo_source_table", total=e.2)
...
step4.4<-function(dt, var_name){
  ...
  #mo_record_vocabulary
  a.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, mo_record_vocabulary)]
  setnames(a.1, "mo_record_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="mo_record_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_record_vocabulary), .N, by=.(year)]
  setnames(a.2, "N", "total")
a.2<-data.table(variable="mo_record_vocabulary", a.2)
  
  #mo_unit
  b.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, mo_unit)]
  setnames(b.1, "mo_unit", "vocabulary")
  b.1<-cbind(b.1, variable="mo_unit")
  b.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_unit), .N, by=.(year)]
  setnames(b.2, "N", "total")
  b.2<-data.table(variable="mo_unit", b.2)
  
  #mo_meaning
  c.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, mo_meaning)]
  setnames(c.1, "mo_meaning", "vocabulary")
  c.1<-cbind(c.1, variable="mo_meaning")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_meaning), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="mo_meaning", c.2)
  
  #mo_origin
  d.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, mo_origin)]
  setnames(d.1, "mo_origin", "vocabulary")
  d.1<-cbind(d.1, variable="mo_origin")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_origin), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="mo_origin", d.2)
  
    #mo_source_table
  e.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, mo_source_table)]
  setnames(e.1, "mo_source_table", "vocabulary")
  e.1<-cbind(e.1, variable="mo_source_table")
  e.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(mo_source_table), .N, by=.(year)]
  setnames(e.2, "N", "total")
  e.2<-data.table(variable="mo_source_table", e.2)
...
dates_count.my<-function(dt, var_name, date_var1, date_var2){
  ...
  if( !(is.null(date_var2))){
    #Transform date_var2 into the right format
...
#Distribution of mo_source_value_by table, column and unit
distribution_mo_unit<-function(dt, var, table_var, column_var, unit_var){
  ...
  setnames(dist, "table", "mo_source_table")
  setnames(dist, "column", "mo_source_column")
  setnames(dist, "unit", "mo_unit")
  return(dist)
}
```
...
step_4_5_check<- function(tables_list){
  ...
  for (i in 1:length(tables_list)){
    ...
    continuous<-df[!is.na(mo_source_table) & !is.na(mo_source_column) & !is.na(mo_source_value) & !is.na(mo_unit),c("mo_source_table", "mo_source_column", "mo_source_value", "mo_unit")]
     continuous<-continuous[!continuous[["mo_unit"]] %like% "DRG|ICD|READ|SNOMED|OPCS|CPT|PCS|NDC|ICP"]
     if (continuous[,.N]>0){
    write.csv(continuous, paste(tmp,"continuous_", tables_list[i],sep="") ,row.names = F)
     }
    rm(continuous)
    
    Res.4.1[[w]]<-step4.1(dt=df, var_name = "mo_meaning")
    Res.4.2[[w]]<-step4.2(dt=df, var_name = "mo_meaning")
    Res.4.3[[w]]<-step4.3(dt=df, var_name = "mo_meaning")
    Res.4.4[[w]]<-step4.4(dt=df, var_name = "mo_meaning")
    Res.p<-df[, c("person_id", "year", "mo_meaning")][!is.na(person_id)][!is.na(mo_meaning)]
      write.csv(Res.p, paste(tmp,"persons_", tables_list[i],sep="") ,row.names = F)
    rm(Res.p)
    
    Res.v<-df[, c("visit_occurrence_id", "year", "mo_meaning")][!is.na(visit_occurrence_id)][!is.na(mo_meaning)]
    if (Res.v[,.N]>0){
      write.csv(Res.v, paste(tmp,"visits_", tables_list[i],sep="") ,row.names = F)
    rm(Res.v)
    }
    Res.dates.m[[w]]<-dates_count.m(dt=df, var_name = "mo_meaning", date_var1 = "mo_date", date_var2 = NULL)
    Res.dates.my[[w]]<-dates_count.my(dt=df, var_name = "mo_meaning", date_var1 = "mo_date", date_var2 = NULL)
    
    Res.p.t[[w]]<-df[!is.na(mo_meaning) & !is.na(person_id), .N]
    Res.p.t.y[[w]]<-df[!is.na(mo_meaning) & !is.na(person_id),.(total=.N), by="year"][!is.na(year)]
    Res.v.t[[w]]<-df[!is.na(mo_meaning) & !is.na(visit_occurrence_id), .N]
    Res.v.t.y[[w]]<-df[!is.na(mo_meaning) & !is.na(visit_occurrence_id),.(total=.N), by="year"][!is.na(year)]
    w<-w+1
    rm(df)
  }
  ...
  continuous_files<-list.files(tmp, pattern="^continuous")
   if (length(continuous_files)>1){
     Res.continuous<-fread(paste0(tmp,continuous_files[1]))
  i<-2
  for (i in 2:length(continuous_files)){
    Res.continuous<-rbind(Res.continuous,fread(paste0(tmp,continuous_files[i])))
  }
  tot_continuous<-Res.continuous[,.N]
   }
  if (length(continuous_files)==1){
     Res.continuous<-fread(paste0(tmp,continuous_files))
     tot_continuous<-Res.continuous[,.N]
  }
if (length(continuous_files)==0){
  ...
  if (!is.null(Res.continuous)){
suppressWarnings(Res.continuous<-Res.continuous[,mo_source_value:= as.numeric(mo_source_value)])  #Transform the mo_source_value into numeric from character
#Check if transformation went well, if not the distribution can not be shown
if(Res.continuous[is.na(mo_source_value),.N]>0){
dist.mo_unit<-"The variable mo_source_value contains non-numeric values and as a result the distribution of this variable cannot be estimated."
} else {
  #Save results for distributions
     #Dist of mo_source_value by table and column and unit
if(Res.continuous[,.N] !=0){
dist.mo_unit<-cbind(variable_name="mo_source_value",distribution_mo_unit(dt=Res.continuous, var="mo_source_value", table_var = "mo_source_table", column_var = "mo_source_column", unit_var = "mo_unit"), total_records=tot_continuous)
} else {dist.mo_unit<-NULL}
}
  rm(Res.continuous)
  } else {dist.mo.unit<-NULL}
 
 #Results for person_id
  persons_files<-list.files(tmp, pattern="^persons")
  #stratified by meaning
  if (length(persons_files)>1){
  Res.p.c<-fread(paste0(tmp,persons_files[1]))
  Res.p.c[, comb:=paste(person_id, mo_meaning, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb:=paste(person_id, mo_meaning, sep="_^_")][!duplicated(comb)]
    Res.p.c<-rbind(Res.p.c,a)
    Res.p.c<-Res.p.c[!duplicated(comb)]
    rm(a)
  }
  Res.p.c[,comb:=NULL]
    Res.p.c<-data.table(variable_name="person_id",Res.p.c[!is.na(mo_meaning), .(count=.N), by="mo_meaning"])
     names(Res.p.c)<-c("variable_name", "meaning", "count")
  Res.p.t<-do.call(rbind, Res.p.t)
  Res.p.t<-sum(Res.p.t)
   
  #stratified by meaning and year
  Res.p.c.y<-fread(paste0(tmp,persons_files[1]))
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "mo_meaning")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "mo_meaning")])][!duplicated(comb_y)]
    Res.p.c.y<-rbind(Res.p.c.y,a)
    Res.p.c.y<-Res.p.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.p.c.y[,comb_y:=NULL]
        Res.p.c.y<-cbind(variable_name="person_id", Res.p.c.y[!is.na(mo_meaning), .(count=.N), by=.(mo_meaning, year)])
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  #stratified by meaning and year
  Res.p.c.y<-fread(paste0(tmp,persons_files))
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "mo_meaning")])][!duplicated(comb_y)]
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  visits_files<-list.files(tmp, pattern="^visits")
  
  #stratified by meaning
  if(length(visits_files)>1){
  Res.v.c<-fread(paste0(tmp,visits_files[1]))
  Res.v.c[, comb:=paste(visit_occurrence_id, mo_meaning, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[, comb:=paste(visit_occurrence_id, mo_meaning, sep="_^_")][!duplicated(comb)]
    Res.v.c<-rbind(Res.v.c,a)
    Res.v.c<-Res.v.c[!duplicated(comb)]
    rm(a)
  }
  Res.v.c[,comb:=NULL]
    Res.v.c<-data.table(variable_name="visit_occurrence_id",Res.v.c[!is.na(mo_meaning), .(count=.N), by="mo_meaning"])
     names(Res.v.c)<-c("variable_name", "meaning", "count")
  Res.v.t<-do.call(rbind, Res.v.t)
  Res.v.t<-sum(Res.v.t)
  
  #stratified by meaning and year
  Res.v.c.y<-fread(paste0(tmp,visits_files[1]))[!is.na(year)]
  Res.v.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "mo_meaning")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[!is.na(year)][, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "mo_meaning")])][!duplicated(comb_y)]
    Res.v.c.y<-rbind(Res.v.c.y,a)
    Res.v.c.y<-Res.v.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.v.c.y[,comb_y:=NULL]
  Res.v.c.y<-cbind(variable_name="visit_occurrence_id", Res.v.c.y[!is.na(mo_meaning), .(count=.N), by=.(mo_meaning, year)])
...
Res.v.c[,comb:=NULL]
  ...
  #stratified by meaning and year
  Res.v.c.y<-fread(paste0(tmp,visits_files))[!is.na(year)]
  Res.v.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "mo_meaning")])][!duplicated(comb_y)]
Res.v.c.y[,comb_y:=NULL]
  Res.v.c.y<-cbind(variable_name="visit_occurrence_id", Res.v.c.y[!is.na(mo_meaning), .(count=.N), by=.(mo_meaning, year)])
Res.v.c.y[,year:=as.integer(year)]
...
if(length(visits_files)==0) {
 ...
 Res.dates.my<-Res.dates.my[meaning !="NA"]
  ...
  output<-list("COUNT1_M"=Res.4.1, "COUNT1_MY"=Res.4.2, "COUNT2_M"=Res.4.3[vocabulary !="NA"], "COUNT2_MY"=Res.4.4[vocabulary !="NA"], "DATES.M"=Res.dates.m, "DATES_MY"=Res.dates.my, "DIST_MO_UNIT"= dist.mo_unit, "DUP"=dup)
  return(output)
}
...
Results.mo<-step_4_5_check(actual_tables)
```

```{r include=FALSE}
 unlink(paste0(mo_dir, "tmp"), recursive = T)
```
...
Conventions:  
**1.** The MEDICAL_OBSERVATIONS table does not contain any conventions that can be checked by the script.   
...
The MEDICAL_OBSERVATIONS table does not contain any conventions that can be checked by the script.
...
Res.count2.m<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$COUNT2_M)
...
write.csv(Res.count2.m, paste(mo_dir,"medical_observations_meaning_2categories.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.m, paste(mo_less,"medical_observations_meaning_2categories_masked.csv",sep="") ,row.names = F)
```
...
Res.count1.m<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$COUNT1_M)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(mo_dir,"medical_observations_meaning_other.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(mo_less,"medical_observations_meaning_other_masked.csv",sep="") ,row.names = F)
...
Res.count2.my<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$COUNT2_MY)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(mo_dir,"medical_observations_meaning_year_2categories.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(mo_less,"medical_observations_meaning_year_2categories_masked.csv",sep=""),row.names = F)
```
...
Res.count1.my<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$COUNT1_MY)
setnames(Res.count1.my, "variable", "variable_name")
...
#Save results as .csv file
write.csv(Res.count1.my, paste(mo_dir,"medical_observations_meaning_year_other.csv",sep=""), row.names = F)
...
datatable(Res.count1.my, option=list(scrollX=TRUE)) %>% formatStyle("percentage",
  background = styleColorBar(range(Res.count1.my.1[["percentage"]]), "#76b82a"),
  backgroundSize = '98% 88%',
  backgroundRepeat = 'no-repeat',
  backgroundPosition = 'center') %>% formatStyle(
  'year', 
...
#Save results as .csv file
write.csv(Res.count1.my, paste(mo_less,"medical_observations_meaning_year_other_masked.csv",sep=""), row.names = F)
```
...
## 3. Distribution of continuous variables and counts of date variables
...
The MEDICAL_OBSERVATION table contains 1 continuous variable:    
**a.** mo_source_value    

Distribution of continuous variables will be stratified by:    
**a.** mo_source_table and mo_source_column and mo_unit      
 
Explanation:  
If total_records(number of records with complete information on `mo_source_table`, `mo_source_column`, `mo_source_value` and `mo_unit`) is smaller than 5, the distribution will not be provided.  

<br>

#### **Distribution stratified by mo_source_table, mo_source_column and, mo_unit**  

```{r dist_mo_unit_result, echo=FALSE, warning=FALSE}
Res.dist.mo.unit<-Results.mo$DIST_MO_UNIT
if(!is.null(Res.dist.mo.unit) & !is.character(Res.dist.mo.unit)){
#output for distribution by box size unit
Res.dist.mo.unit<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$DIST_MO_UNIT)
Res.dist.mo.unit<-data.table(Res.dist.mo.unit[,1:5], 
                             Res.dist.mo.unit[, lapply(.SD, FUN= function(x) round(x, 2)), .SDcols=c('mean', 'SD', 'median', 'IQR', 'skewness', 'kurtosis', 'total_records')])


if (Res.dist.mo.unit[total_records<5, .N]>0){
  comment_Res.dist.mo.unit<-"The distribution of mo_source_value for one or more meaning categories cannot be displayed since the number of records with complete information in mo_source_table, mo_source_column, mo_source_value, and mo_unit is smaller than 5."
Res.dist.mo.unit_masked<-Res.dist.mo.unit[total_records>5]

if (Res.dist.mo.unit_masked[,.N]>0){
  print(comment_Res.dist.mo.unit)
  datatable(Res.dist.mo.unit_masked, options = list(scrollX=T))
  } else {
print("The distribution of mo_source_value cannot be displayed since the number of records with complete information in mo_source_table, mo_source_column, mo_source_value, and mo_unit is smaller than 5.")
  }

} else {
  datatable(Res.dist.mo.unit, options = list(scrollX=T))}
}

if (is.null(Res.dist.mo.unit)){
  print("The distribution of mo_source_value cannot be displayed due to complete missingness of one of the variables: mo_source_table, mo_source_column, mo_source_value, and mo_unit.")
} 
if (is.character(Res.dist.mo.unit)){
  print(Res.dist.mo.unit)
}


#Save results as .csv file
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
write.csv(Res.dist.mo.unit, paste(mo_dir,"medical_observations_mo_value_dist.csv",sep="") ,row.names = F)
}

#Save results as .csv file
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
if (Res.dist.mo.unit[total_records<5, .N]==0){
write.csv(Res.dist.mo.unit, paste(mo_less,"medical_observations_mo_value_dist_masked.csv",sep="") ,row.names = F)
} 
}
...
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
  if (Res.dist.mo.unit[total_records<5, .N]>0){
    if(Res.dist.mo.unit_masked[,.N]>0){
write.csv(Res.dist.mo.unit_masked, paste(mo_less,"medical_observations_mo_value_dist_masked.csv",sep="") ,row.names = F)
  }
  }
}
...
```{r graph_dist_mo_unit, echo=FALSE, warning=FALSE}
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
  if (Res.dist.mo.unit[total_records<5, .N]==0){
fig.dist.mo.u<-vector(mode="list", length=length(unique(Res.dist.mo.unit[["mo_source_table"]])))
for(i in 1:length(unique(Res.dist.mo.unit[["mo_source_table"]]))) {
  fig.dist.mo.u[[i]]<-ggplotly(ggplot(Res.dist.mo.unit[mo_source_table==unique(Res.dist.mo.unit[["mo_source_table"]])[i]],aes(x = mo_unit, y = mean, group=mo_source_column)) +
                           geom_line(aes(color=mo_source_column)) +
                           geom_point(aes(color=mo_source_column)) +
                              theme_classic() +
                               ggtitle(paste("Source table:",unique(Res.dist.mo.unit[["mo_source_table"]])[i])) + 
                               xlab("Unit") +
                               ylab("Mean")+
                               theme(axis.text.x = element_text(angle = 45),
                                     axis.title.x = element_text(colour = "#76b82a"),
...
```{r display_graph_dist_mo_unit, echo=FALSE}
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
  if (Res.dist.mo.unit[total_records<5, .N]==0){
htmltools::tagList(list(fig.dist.mo.u))
  }
}
```

```{r graph_dist_mo_unit_masked, echo=FALSE, warning=FALSE}
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
  if (Res.dist.mo.unit[total_records<5, .N]>0){
  if (Res.dist.mo.unit_masked[,.N]>0){
fig.dist.mo.u<-vector(mode="list", length=length(unique(Res.dist.mo.unit_masked[["mo_source_table"]])))
for(i in 1:length(unique(Res.dist.mo.unit_masked[["mo_source_table"]]))) {
  fig.dist.mo.u[[i]]<-ggplotly(ggplot(Res.dist.mo.unit_masked[mo_source_table==unique(Res.dist.mo.unit_masked[["mo_source_table"]])[i]],aes(x = mo_unit, y = mean, group=mo_source_column)) +
                           geom_line(aes(color=mo_source_column)) +
                           geom_point(aes(color=mo_source_column)) +
                              theme_classic() +
                               ggtitle(paste("Source table:",unique(Res.dist.mo.unit_masked[["mo_source_table"]])[i])) + 
                               xlab("Unit") +
                               ylab("Mean")+
                               theme(axis.text.x = element_text(angle = 45),
                                     axis.title.x = element_text(colour = "#76b82a"),
...
```{r display_graph_dist_mo_unit_masked, echo=FALSE}
if(!(is.null(Res.dist.mo.unit)) & !is.character(Res.dist.mo.unit)){
  if (Res.dist.mo.unit[total_records<5, .N]>0){
  if (Res.dist.mo.unit_masked[,.N]>0){
htmltools::tagList(list(fig.dist.mo.u))
...
Res.dates.m<-data.table(table_name="MEDICAL_OBSERVATIONS", Results.mo$DATES.M)
...
if (nrow(Res.dates.m[(count ==0 & total ==0)])>0) {
  print(paste("Attention! There is(are) ", nrow(Res.dates.m[(count ==0 & total ==0)]), "row(s) with a zero value for both count and total. Those will not be displayed in the tables or graphs. This happens when a variable is completely missing for a particular meaning category."))
}
#Remove rows where both count and total are 0
...
#Save results as .csv file
write.csv(Res.dates.m, paste(mo_dir,"medical_observations_meaning_dates.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(mo_less,"medical_observations_meaning_dates_masked.csv",sep=""), row.names = F)
```
...
Res.dates.my<-data.table(table_name="MEDICAL_OBSERVATIONS",Results.mo$DATES_MY)
...
if (nrow(Res.dates.my[(count ==0 & total ==0)])>0) {
  print(paste("Attention! There is(are) ", nrow(Res.dates.my[(count ==0 & total ==0)]), "row(s) with a zero value for both count and total. Those will not be displayed in the tables or graphs. This happens when a variable is completely missing for a particular meaning category and year."))
}
#Remove rows where both count and total are 0
...
#Save results as .csv file
write.csv(Res.dates.my, paste(mo_dir,"medical_observations_meaning_year_dates.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(mo_less,"medical_observations_meaning_year_dates_masked.csv",sep=""),row.names = F)
```
...
Numerator= count  
Denominator= total   
Year= retrieved from the year part of the variable `mo_date`.

```{r calculations_table, echo=FALSE}
calculations_mo<-data.table(rbind(
  cbind(calculation= "Counts stratified by meaning(other variables)", variable_name=c("person_id", "visit_occurrence_id", "mo_code", "mo_source_column", "mo_source_value"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category", "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
    cbind(calculation= "Counts stratified by meaning and year(other variables)", variable_name=c("person_id", "visit_occurrence_id", "mo_code", "mo_source_column", "mo_source_value"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
      cbind(calculation= "Counts stratified by meaning(2 or more categories)", variable_name=c("mo_record_vocabulary", "mo_unit", "mo_meaning", "mo_origin", "mo_source_table"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning","Number of total observations with a recorded meaning")),
      cbind(calculation= "Counts stratified by meaning and year(2 or more categories)", variable_name=c("mo_record_vocabulary", "mo_unit", "mo_meaning", "mo_origin","mo_source_table"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year","Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning", variable_name=c("mo_date"), numerator=c( "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning")),
  cbind(calculation= "Date counts stratified by meaning and year", variable_name=c("mo_date"), numerator=c( "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year"))
))
datatable(calculations_mo)
```
...
**1.** MEDICAL_OBSERVATIONS.html: Rmarkdown report      
**2.** `medical_observations_meaning_2categories.csv`: counts of variables with 2 or more categories stratified by meaning      
**3.** `medical_observations_meaning_other.csv`: counts of other variables stratified by meaning          
**4.** `medical_observations_meaning_year_2categories.csv`: counts of variables with 2 or more categories stratified by meaning and year         
**5.** `medical_observations_meaning_year_other.csv`: counts of other variables stratified by meaning and year         
**6.** `medical_observations_meaning_dates.csv`: counts of date variables stratified by meaning           
**7.** `medical_observations_meaning_year_dates.csv`: counts of date variables stratified by meaning and year  
**8.** `medical_observations_mo_value_dist.csv`: distribution of mo_source_value stratified by mo_source_table, mo_source_column and mo_source_unit  
**9.** Masked:      
&nbsp;&nbsp;&nbsp;&nbsp;***a.*** `medical_observations_meaning_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning where counts smaller than 5 are masked          
&nbsp;&nbsp;&nbsp;&nbsp;***b.*** `medical_observations_meaning_other_masked.csv`: counts of other variables stratified by meaning where counts smaller than 5 are masked       
&nbsp;&nbsp;&nbsp;&nbsp;***c.*** `medical_observations_meaning_year_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning and year where counts smaller than 5 are masked            
&nbsp;&nbsp;&nbsp;&nbsp;***d.*** `medical_observations_meaning_year_other_masked.csv`: counts of other variables stratified by meaning and year where counts smaller than 5 are masked     
&nbsp;&nbsp;&nbsp;&nbsp;***e.*** `medical_observations_meaning_dates_masked.csv`: counts of date variables stratified by meaning where counts smaller than 5 are masked               
&nbsp;&nbsp;&nbsp;&nbsp;***f.*** `medical_observations_meaning_year_dates_masked.csv`: counts of date variables stratified by meaning and year where counts smaller than 5 are masked    
&nbsp;&nbsp;&nbsp;&nbsp;***g.*** `medical_observations_mo_value_dist_masked.csv`: distribution of mo_source_value stratified by mo_source_table, mo_source_column and mo_source_unit where rows with number of total records smaller than 5 are removed    
