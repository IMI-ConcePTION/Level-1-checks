---
title: 'CONCEPTION - Level 1 checks: Step 4 to 5 (PROCEDURES)'
output: 
  html_document:
    theme: spacelab
    toc: true
...
if ("PROCEDURES" %in% list.files(output_dir)){
  procedures_dir<-paste(output_dir, "PROCEDURES/", sep="")
  procedures_less<-paste(procedures_dir, "Masked/", sep="")
  dir.create(paste(procedures_dir,"tmp", sep=""))
tmp<-paste(procedures_dir, "tmp/", sep="")
} else {
#Create the EVENTS folder in the output dir
dir.create(paste(output_dir, "PROCEDURES", sep=""))
  procedures_dir<-paste(output_dir, "PROCEDURES/", sep="")
  dir.create(paste(procedures_dir,"Masked", sep=""))
procedures_less<-paste(procedures_dir, "Masked/", sep="")
dir.create(paste(procedures_dir,"tmp", sep=""))
tmp<-paste(procedures_dir, "tmp/", sep="")
}
...
total_var<-as.data.table(cbind(table_name="PROCEDURES", 
...
#List of actual tables that match PROCEDURES pattern
actual_tables<-directory_CDM[grepl(pattern="PROCEDURES", x=directory_CDM)]
...
step4.1<-function(dt, var_name){
  ...
  #procedure_code
  c.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("procedure_code"), by=.(meaning)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning"), measure.vars=colnames(c.1)[-1]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(procedure_code), .N]
  c.2<-data.table(variable="procedure_code", total=c.2)
...
step4.2<-function(dt, var_name){
  ...
  #procedure_code
  c.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("procedure_code"), by=.(meaning, year)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning", "year"), measure.vars=colnames(c.1)[-c(1:2)]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(procedure_code), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="procedure_code", c.2)
...
step4.3<-function(dt, var_name){
  ...
  a.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, procedure_code_vocabulary)]
  setnames(a.1, "procedure_code_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="procedure_code_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(procedure_code_vocabulary), .N]
  a.2<-data.table(variable="procedure_code_vocabulary", total=a.2)

  
  #meaning_of_procedure
  b.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, meaning_of_procedure)]
  setnames(b.1, "meaning_of_procedure", "vocabulary")
  b.1<-cbind(b.1, variable="meaning_of_procedure")
  b.2<-dt[complete.cases(meaning) & complete.cases(meaning_of_procedure), .N]
  b.2<-data.table(variable="meaning_of_procedure", total=b.2)
  
  
  #origin_of_procedure
  c.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, origin_of_procedure)]
  setnames(c.1, "origin_of_procedure", "vocabulary")
  c.1<-cbind(c.1, variable="origin_of_procedure")
  c.2<-dt[complete.cases(meaning) & complete.cases(origin_of_procedure), .N]
  c.2<-data.table(variable="origin_of_procedure", total=c.2)
...
step4.4<-function(dt, var_name, date_var){

  dt[, meaning:= dt[[var_name]]]#Duplicate the meaning variable
  
  #procedure_code_vocabulary
  a.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, procedure_code_vocabulary)]
  setnames(a.1, "procedure_code_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="procedure_code_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(procedure_code_vocabulary), .N, by=.(year)]
  setnames(a.2, "N", "total")
a.2<-data.table(variable="procedure_code_vocabulary", a.2)
  
  #meaning_of_procedure
  b.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, meaning_of_procedure)]
  setnames(b.1, "meaning_of_procedure", "vocabulary")
  b.1<-cbind(b.1, variable="meaning_of_procedure")
  b.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(meaning_of_procedure), .N, by=.(year)]
  setnames(b.2, "N", "total")
  b.2<-data.table(variable="meaning_of_procedure", b.2)
  
  #origin_of_procedure
  c.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, origin_of_procedure)]
  setnames(c.1, "origin_of_procedure", "vocabulary")
  c.1<-cbind(c.1, variable="origin_of_procedure")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(origin_of_procedure), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="origin_of_procedure", c.2)
...
step_4_5_check<- function(tables_list){
  ...
  for (i in 1:length(tables_list)){
    ...
    df[,year:=substr(procedure_date, 1, 4)]
...
#Create a combination variable by combining all columns, keep only that variable
    ...
    Res.4.1[[w]]<-step4.1(dt=df, var_name = "meaning_of_procedure")
    Res.4.2[[w]]<-step4.2(dt=df, var_name = "meaning_of_procedure")
    Res.4.3[[w]]<-step4.3(dt=df, var_name = "meaning_of_procedure")
    Res.4.4[[w]]<-step4.4(dt=df, var_name = "meaning_of_procedure")
    
    Res.p<-df[, c("person_id", "year", "meaning_of_procedure")][!is.na(person_id)][!is.na(meaning_of_procedure)]
      write.csv(Res.p, paste(tmp,"persons_", tables_list[i],sep="") ,row.names = F)
    rm(Res.p)
    
    Res.v<-df[, c("visit_occurrence_id", "year", "meaning_of_procedure")][!is.na(visit_occurrence_id)][!is.na(meaning_of_procedure)]
    if (Res.v[,.N]>0){
      write.csv(Res.v, paste(tmp,"visits_", tables_list[i],sep="") ,row.names = F)
    rm(Res.v)
    }
    
    Res.dates.m[[w]]<-dates_count.m(dt=df, var_name = "meaning_of_procedure", date_var1 = "procedure_date", date_var2 = NULL)
    Res.dates.my[[w]]<-dates_count.my(dt=df, var_name = "meaning_of_procedure", date_var1 = "procedure_date", date_var2 = NULL)
    
    Res.p.t[[w]]<-df[!is.na(meaning_of_procedure) & !is.na(person_id), .N]
    Res.p.t.y[[w]]<-df[!is.na(meaning_of_procedure) & !is.na(person_id),.(total=.N), by="year"][!is.na(year)]
    Res.v.t[[w]]<-df[!is.na(meaning_of_procedure) & !is.na(visit_occurrence_id), .N]
    Res.v.t.y[[w]]<-df[!is.na(meaning_of_procedure) & !is.na(visit_occurrence_id),.(total=.N), by="year"][!is.na(year)]
    w<-w+1
    rm(df)
  }
  ...
  Res.p.c[, comb:=paste(person_id, meaning_of_procedure, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb:=paste(person_id, meaning_of_procedure, sep="_^_")][!duplicated(comb)]
    Res.p.c<-rbind(Res.p.c,a)
    Res.p.c<-Res.p.c[!duplicated(comb)]
    rm(a)
  }
  Res.p.c[,comb:=NULL]
    Res.p.c<-data.table(variable_name="person_id",Res.p.c[!is.na(meaning_of_procedure), .(count=.N), by="meaning_of_procedure"])
  ...
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_procedure")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_procedure")])][!duplicated(comb_y)]
    Res.p.c.y<-rbind(Res.p.c.y,a)
    Res.p.c.y<-Res.p.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.p.c.y[,comb_y:=NULL]
        Res.p.c.y<-cbind(variable_name="person_id", Res.p.c.y[!is.na(meaning_of_procedure), .(count=.N), by=.(meaning_of_procedure, year)])
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  setnames(Res.p.y, "meaning_of_procedure", "meaning")
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  Res.v.c[, comb:=paste(visit_occurrence_id, meaning_of_procedure, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[, comb:=paste(visit_occurrence_id, meaning_of_procedure, sep="_^_")][!duplicated(comb)]
    Res.v.c<-rbind(Res.v.c,a)
    Res.v.c<-Res.v.c[!duplicated(comb)]
    rm(a)
  }
  Res.v.c[,comb:=NULL]
    Res.v.c<-data.table(variable_name="visit_occurrence_id",Res.v.c[!is.na(meaning_of_procedure), .(count=.N), by="meaning_of_procedure"])
  ...
  Res.v.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_procedure")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[!is.na(year)][, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_procedure")])][!duplicated(comb_y)]
    Res.v.c.y<-rbind(Res.v.c.y,a)
    Res.v.c.y<-Res.v.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.v.c.y[,comb_y:=NULL]
  Res.v.c.y<-cbind(variable_name="visit_occurrence_id", Res.v.c.y[!is.na(meaning_of_procedure), .(count=.N), by=.(meaning_of_procedure, year)])
...
#Res.v.c.y, Res.v.t.y counts and total stratified by meaning and year: visit_occurrence_id
  ...
  setnames(Res.v.y, "meaning_of_procedure", "meaning")
...
if(length(visits_files)==0) {
 ...
 Res.dates.my<-Res.dates.my[meaning !="NA"]
  ...
  output<-list("COUNT1_M"=Res.4.1, "COUNT1_MY"=Res.4.2, "COUNT2_M"=Res.4.3[vocabulary !="NA"], "COUNT2_MY"=Res.4.4[vocabulary !="NA"], "DATES.M"=Res.dates.m, "DATES_MY"=Res.dates.my, "DUP"=dup)
  return(output)
}
...
Results.procedures<-step_4_5_check(actual_tables)
```

```{r include=FALSE}
 unlink(paste0(procedures_dir, "tmp"), recursive = T)
```
...
Conventions:  
**1.** There are no conventions for the PROCEDURES table that can be checked by the script.  
...
The PROCEDURES table does not contain any conventions that can be checked by the script.   
...
**a.** procedure_code_vocabulary  
**b.** meaning_of_procedure  
**c.** origin_of_procedure  
...
Res.count2.m<-data.table(table_name="PROCEDURES", Results.procedures$COUNT2_M)
...
write.csv(Res.count2.m, paste(procedures_dir,"procedures_meaning_2categories.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.m, paste(procedures_less,"procedures_meaning_2categories_masked.csv",sep="") ,row.names = F)
```
...
Res.count1.m<-data.table(table_name="PROCEDURES", Results.procedures$COUNT1_M)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(procedures_dir,"procedures_meaning_other.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(procedures_less,"procedures_meaning_other_masked.csv",sep="") ,row.names = F)
```
...
Res.count2.my<-data.table(table_name="PROCEDURES", Results.procedures$COUNT2_MY)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(procedures_dir,"procedures_meaning_year_2categories.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(procedures_less,"procedures_meaning_year_2categories_masked.csv",sep=""),row.names = F)
```
...
Res.count1.my<-data.table(table_name="PROCEDURES", Results.procedures$COUNT1_MY)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(procedures_dir,"procedures_meaning_year_other.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(procedures_less,"procedures_meaning_year_other_masked.csv",sep=""), row.names = F)
```
...
The PROCEDURES table does not contain continous variables.
<br>
...
Res.dates.m<-data.table(table_name="PROCEDURES", Results.procedures$DATES.M)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(procedures_dir,"procedures_meaning_dates.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(procedures_less,"procedures_meaning_dates.csv",sep=""), row.names = F)
```


```{r graph_count.dates_m, echo=FALSE}
if (Res.dates.m[count == "<5", .N] != 0){
...
Res.dates.my<-data.table(table_name="PROCEDURES",Results.procedures$DATES_MY)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(procedures_dir,"procedures_meaning_year_dates.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(procedures_less,"procedures_meaning_year_dates_masked.csv",sep=""),row.names = F)
```
...
Numerator= count  
Denominator= total   
Year= retrieved from the year part of the variable `procedure_date`  

```{r echo=FALSE}
calculations_procedures<-data.table(rbind(
  cbind(calculation= "Counts stratified by meaning(other variables)", variable_name=c("person_id", "visit_occurrence_id", "procedure_code"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
    cbind(calculation= "Counts stratified by meaning and year(other variables)", variable_name=c("person_id", "visit_occurrence_id", "procedure_code"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
      cbind(calculation= "Counts stratified by meaning(2 or more categories)", variable_name=c("procedure_code_vocabulary", "meaning_of_procedure", "origin_of_procedure"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
      cbind(calculation= "Counts stratified by meaning and year(2 or more categories)", variable_name=c("procedure_code_vocabulary", "meaning_of_procedure", "origin_of_procedure"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning", variable_name=c("procedure_date"), numerator=c( "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning")),
  cbind(calculation= "Date counts stratified by meaning and year", variable_name=c("procedure_date"), numerator=c( "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year"))
))
datatable(calculations_procedures)
```
...
PROCEDURES     
**1.** PROCEDURES.html: Rmarkdown report      
**2.** `procedures_meaning_2categories.csv`: counts of variables with 2 or more categories stratified by meaning      
**3.** `procedures_meaning_2categories.csv`: counts of other variables stratified by meaning    
**4.** `procedures_meaning_year_2categories.csv`: counts of variables with 2 or more categories stratified by meaning and year         
**5.** `procedures_meaning_year_other.csv`: counts of other variables stratified by meaning and year         
**6.** `procedures_meaning_dates.csv`: counts of date variables stratified by meaning       
**7.** `procedures_meaning_year_dates.csv`: counts of date variables stratified by meaning and year    
**8.** Masked:      
&nbsp;&nbsp;&nbsp;&nbsp;***a.*** `procedures_meaning_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning where counts smaller than 5 are masked          
&nbsp;&nbsp;&nbsp;&nbsp;***b.*** `procedures_meaning_other_masked.csv`: counts of other variables stratified by meaning where counts smaller than 5 are masked       
&nbsp;&nbsp;&nbsp;&nbsp;***c.*** `procedures_meaning_year_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning and year where counts smaller than 5 are masked            
&nbsp;&nbsp;&nbsp;&nbsp;***d.*** `procedures_meaning_year_other_masked.csv`: counts of other variables stratified by meaning and year where counts smaller than 5 are masked     
&nbsp;&nbsp;&nbsp;&nbsp;***e.*** `procedures_meaning_dates.csv`: counts of date variables stratified by meaning where counts smaller than 5 are masked               
&nbsp;&nbsp;&nbsp;&nbsp;***f.*** `procedures_meaning_year_dates_masked.csv`: counts of date variables stratified by meaning and year where counts smaller than 5 are masked    
