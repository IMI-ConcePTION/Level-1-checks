---
title: 'CONCEPTION - Level 1 checks: Step 4 to 5 (VISIT_OCCURRENCE)'
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r include=FALSE}
if ("VISIT_OCCURRENCE" %in% list.files(output_dir)){
  visits_dir<-paste(output_dir, "VISIT_OCCURRENCE/", sep="")
  visits_less<-paste(visits_dir, "Masked/", sep="")
   dir.create(paste(visits_dir,"tmp", sep=""))
tmp<-paste(visits_dir, "tmp/", sep="")
} else {
#Create the VISIT_OCCURRENCE folder in the output dir
dir.create(paste(output_dir, "VISIT_OCCURRENCE", sep=""))
  visits_dir<-paste(output_dir, "VISIT_OCCURRENCE/", sep="")
  dir.create(paste(visits_dir,"Masked", sep=""))
visits_less<-paste(visits_dir, "Masked/", sep="")
   dir.create(paste(visits_dir,"tmp", sep=""))
tmp<-paste(visits_dir, "tmp/", sep="")
}
...
total_var<-as.data.table(cbind(table_name="VISIT_OCCURRENCE", 
...
#List of actual tables that match VISIT_OCCURRENCE pattern
actual_tables<-directory_CDM[grepl(pattern="VISIT_OCCURRENCE", x=directory_CDM)]
```

```{r convention_check, include=FALSE}
#Check conventions
convention_check_visit<-function(visits_list){
 if (length(visits_list)>1){
  Res.v<-fread(paste0(tmp,visits_list[1]))
i<-2
  for (i in 2:length(visits_list)){
    a<-fread(paste0(tmp,visits_list[i]))
    Res.v<-rbind(Res.v,a)
    rm(a)
  }
Res.v<-Res.v[duplicated(visit_occurrence_id),.N]
  } 
  
    if (length(visits_list)==1){
  Res.v<-fread(paste0(tmp,visits_list))
  Res.v<-Res.v[duplicated(visit_occurrence_id),.N]
    } 
  ...
  if (con.1 >0){
    ...
    comment<-paste("Convention 1 is not satisfied.There is(are)", con.1, 
                   "duplicated visit occurrence id(s) in the table VISIT_OCCURRENCE.")
  } else {
    error<-FALSE
...
step4.1<-function(dt, var_name){
  ...
  #speciality_of_visit
  c.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("speciality_of_visit"), by=.(meaning)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning"), measure.vars=colnames(c.1)[-1]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(speciality_of_visit), .N]
  c.2<-data.table(variable="speciality_of_visit", total=c.2)
  
  #status_at_discharge
  d.1<-dt[complete.cases(meaning),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("status_at_discharge"), by=.(meaning)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning"), measure.vars=colnames(d.1)[-1]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(status_at_discharge), .N]
  d.2<-data.table(variable="status_at_discharge", total=d.2)
...
step4.2<-function(dt, var_name){
  ...
  #speciality_of_visit
  c.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("speciality_of_visit"), by=.(meaning, year)] 
  c.1<-data.table::melt(c.1, id.vars=c("meaning", "year"), measure.vars=colnames(c.1)[-c(1:2)]) #from wide to long  
  setnames(c.1, "value", "count")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(speciality_of_visit), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="speciality_of_visit", c.2)

  #status_at_discharge
  d.1<-dt[complete.cases(meaning) & complete.cases(year),lapply(.SD, FUN=function(x) sum(complete.cases(x))), .SDcols=c("status_at_discharge"), by=.(meaning, year)] 
  d.1<-data.table::melt(d.1, id.vars=c("meaning", "year"), measure.vars=colnames(d.1)[-c(1:2)]) #from wide to long  
  setnames(d.1, "value", "count")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(status_at_discharge), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="status_at_discharge", d.2)
...
step4.3<-function(dt, var_name){
  ...
  a.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, speciality_of_visit_vocabulary)]
  setnames(a.1, "speciality_of_visit_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="speciality_of_visit_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(speciality_of_visit_vocabulary), .N]
  a.2<-data.table(variable="speciality_of_visit_vocabulary", total=a.2)

  #status_at_discharge_vocabulary
  b.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, status_at_discharge_vocabulary)]
  setnames(b.1, "status_at_discharge_vocabulary", "vocabulary")
  b.1<-cbind(b.1, variable="status_at_discharge_vocabulary")
  b.2<-dt[complete.cases(meaning) & complete.cases(status_at_discharge_vocabulary), .N]
  b.2<-data.table(variable="status_at_discharge_vocabulary", total=b.2)

  #meaning_of_visit
  c.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, meaning_of_visit)]
  setnames(c.1, "meaning_of_visit", "vocabulary")
  c.1<-cbind(c.1, variable="meaning_of_visit")
  c.2<-dt[complete.cases(meaning) & complete.cases(meaning_of_visit), .N]
  c.2<-data.table(variable="meaning_of_visit", total=c.2)
  
  
  #origin_of_visit
  d.1<-dt[complete.cases(meaning), .(count=.N), by=.(meaning, origin_of_visit)]
  setnames(d.1, "origin_of_visit", "vocabulary")
  d.1<-cbind(d.1, variable="origin_of_visit")
  d.2<-dt[complete.cases(meaning) & complete.cases(origin_of_visit), .N]
  d.2<-data.table(variable="origin_of_visit", total=d.2)
...
step4.4<-function(dt, var_name){
  ...
  #speciality_of_visit_vocabulary
  a.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, speciality_of_visit_vocabulary)]
  setnames(a.1, "speciality_of_visit_vocabulary", "vocabulary")
  a.1<-cbind(a.1, variable="speciality_of_visit_vocabulary")
  a.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(speciality_of_visit_vocabulary), .N, by=.(year)]
  setnames(a.2, "N", "total")
a.2<-data.table(variable="speciality_of_visit_vocabulary", a.2)
  
  #status_at_discharge_vocabulary
  b.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, status_at_discharge_vocabulary)]
  setnames(b.1, "status_at_discharge_vocabulary", "vocabulary")
  b.1<-cbind(b.1, variable="status_at_discharge_vocabulary")
  b.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(status_at_discharge_vocabulary), .N, by=.(year)]
  setnames(b.2, "N", "total")
  b.2<-data.table(variable="status_at_discharge_vocabulary", b.2)
  
  #meaning_of_visit
  c.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, meaning_of_visit)]
  setnames(c.1, "meaning_of_visit", "vocabulary")
  c.1<-cbind(c.1, variable="meaning_of_visit")
  c.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(meaning_of_visit), .N, by=.(year)]
  setnames(c.2, "N", "total")
  c.2<-data.table(variable="meaning_of_visit", c.2)
  
  #origin_of_visit
  d.1<-dt[complete.cases(meaning) & complete.cases(year), .(count=.N), by=.(meaning, year, origin_of_visit)]
  setnames(d.1, "origin_of_visit", "vocabulary")
  d.1<-cbind(d.1, variable="origin_of_visit")
  d.2<-dt[complete.cases(meaning) & complete.cases(year) & complete.cases(origin_of_visit), .N, by=.(year)]
  setnames(d.2, "N", "total")
  d.2<-data.table(variable="origin_of_visit", d.2)
...
#Check if date_var2 is completely empty
  ...
  results<-list("count"=a.1, "total"=a.2)
    ...
    dt[, year:= lubridate::year(dt[[date_var1]])]
...
step_4_5_check<- function(tables_list){
  ...
  Res.v.t.y<-list() #total number of visits with recorded meaning and yea
  w<-1
  for (i in 1:length(tables_list)){
   
    #load table
    df<- fread(paste(path_dir, tables_list[i], sep=""), stringsAsFactors = FALSE, colClasses = "character")
    df<-df[, lapply(.SD, FUN=function(x) gsub("^$|^ $", NA, x))] #replace empty spaces with NA
    
    df[,year:=substr(visit_start_date, 1, 4)]
    ...
    #Save counts 
    Res.4.1[[w]]<-step4.1(dt=df, var_name = "meaning_of_visit")
    Res.4.2[[w]]<-step4.2(dt=df, var_name = "meaning_of_visit")
    Res.4.3[[w]]<-step4.3(dt=df, var_name = "meaning_of_visit")
    Res.4.4[[w]]<-step4.4(dt=df, var_name = "meaning_of_visit")
    
    Res.p<-df[!duplicated(person_id), c("person_id", "year", "meaning_of_visit")][!is.na(person_id)][!is.na(meaning_of_visit)]
    write.csv(Res.p, paste(tmp,"persons_", tables_list[i],sep="") ,row.names = F)
    rm(Res.p)
    Res.v<-df[, c("visit_occurrence_id", "year", "meaning_of_visit")][!is.na(visit_occurrence_id)] #will be used to check conventions as well as unique counts of visits
    write.csv(Res.v, paste(tmp,"visits_", tables_list[i],sep="") ,row.names = F)
    rm(Res.v)
    
    Res.dates.m[[w]]<-dates_count.m(dt=df, var_name = "meaning_of_visit", date_var1 = "visit_start_date", date_var2 = "visit_end_date")
    Res.dates.my[[w]]<-dates_count.my(dt=df, var_name = "meaning_of_visit", date_var1 = "visit_start_date", date_var2 = "visit_end_date")

    Res.p.t[[w]]<-df[!is.na(meaning_of_visit) & !is.na(person_id), .N]
    Res.p.t.y[[w]]<-df[!is.na(meaning_of_visit) & !is.na(person_id),.(total=.N), by="year"][!is.na(year)]
    Res.v.t[[w]]<-df[!is.na(meaning_of_visit) & !is.na(visit_occurrence_id), .N]
    Res.v.t.y[[w]]<-df[!is.na(meaning_of_visit) & !is.na(visit_occurrence_id),.(total=.N), by="year"][!is.na(year)]
    w<-w+1
    rm(df)
  }
  ...
  Res.convention<- data.table(table_name="VISIT_OCCURRENCE", convention_check_visit(visits_files))
  ...
  Res.p.c[, comb:=paste(person_id, meaning_of_visit, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb:=paste(person_id, meaning_of_visit, sep="_^_")][!duplicated(comb)]
    Res.p.c<-rbind(Res.p.c,a)
    Res.p.c<-Res.p.c[!duplicated(comb)]
    rm(a)
  }
  Res.p.c[,comb:=NULL]
    Res.p.c<-data.table(variable_name="person_id",Res.p.c[!is.na(meaning_of_visit), .(count=.N), by="meaning_of_visit"])
  ...
  Res.p.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_visit")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(persons_files)){
    a<-fread(paste0(tmp,persons_files[i]))
    a<-a[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("person_id", "year", "meaning_of_visit")])][!duplicated(comb_y)]
    Res.p.c.y<-rbind(Res.p.c.y,a)
    Res.p.c.y<-Res.p.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.p.c.y[,comb_y:=NULL]
        Res.p.c.y<-cbind(variable_name="person_id", Res.p.c.y[!is.na(meaning_of_visit), .(count=.N), by=.(meaning_of_visit, year)])
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  setnames(Res.p.y, "meaning_of_visit", "meaning")
...
#Res.p.c.y, Res.p.t.y counts and total stratified by meaning and year: person_id
  ...
  Res.v.c[, comb:=paste(visit_occurrence_id, meaning_of_visit, sep="_^_")][!duplicated(comb)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[, comb:=paste(visit_occurrence_id, meaning_of_visit, sep="_^_")][!duplicated(comb)]
    Res.v.c<-rbind(Res.v.c,a)
    Res.v.c<-Res.v.c[!duplicated(comb)]
    rm(a)
  }
  Res.v.c[,comb:=NULL]
    Res.v.c<-data.table(variable_name="visit_occurrence_id",Res.v.c[!is.na(meaning_of_visit), .(count=.N), by="meaning_of_visit"])
  ...
  Res.v.c.y[, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_visit")])][!duplicated(comb_y)]
i<-2
  for (i in 2:length(visits_files)){
    a<-fread(paste0(tmp,visits_files[i]))
    a<-a[!is.na(year)][, comb_y := Reduce(function(...) paste(..., sep = "_^_"), .SD[, c("visit_occurrence_id", "year", "meaning_of_visit")])][!duplicated(comb_y)]
    Res.v.c.y<-rbind(Res.v.c.y,a)
    Res.v.c.y<-Res.v.c.y[!duplicated(comb_y)]
    rm(a)
  }
  Res.v.c.y[,comb_y:=NULL]
  Res.v.c.y<-cbind(variable_name="visit_occurrence_id", Res.v.c.y[!is.na(meaning_of_visit), .(count=.N), by=.(meaning_of_visit, year)])
...
#Res.v.c.y, Res.v.t.y counts and total stratified by meaning and year: visit_occurrence_id
  ...
  setnames(Res.v.y, "meaning_of_visit", "meaning")
...
```{r run_script, echo=FALSE}
Results.visit_occurrence<-step_4_5_check(actual_tables)
```

```{r include=FALSE}
 unlink(paste0(visits_dir, "tmp"), recursive = T)
```


```{r include=FALSE}
rm(convention_check_visit, combine_counts1.m, combine_counts1.my, combine_counts2.m, combine_counts2.my, dates_count.m, dates_count.my, step_4_5_check, step4.1, step4.2, step4.3, step4.4)
```
...
Conventions:  
**1.** `visit_occurrence_id` is a primary key (no duplicates are allowed).    
...
dup<-Results.visit_occurrence$DUP
...
Res.convention<-Results.visit_occurrence$CONVENTION
...
**a.** specialty_of_visit_vocabulary  
**b** status_at_discharge_vocabulary  
**c.** meaning_of_visit  
**d.** origin_of_visit  
...
**c.** specialty_of_visit   
**d.** status_at_discharge     
...
suppressPackageStartupMessages(library(DT))
Res.count2.m<-data.table(table_name="VISIT_OCCURRENCE", Results.visit_occurrence$COUNT2_M)
...
write.csv(Res.count2.m, paste(visits_dir,"visits_meaning_2categories.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.m, paste(visits_less,"visits_meaning_2categories_masked.csv",sep="") ,row.names = F)
```
...
suppressPackageStartupMessages(library(DT))
Res.count1.m<-data.table(table_name="VISIT_OCCURRENCE", Results.visit_occurrence$COUNT1_M)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(visits_dir,"visits_meaning_other.csv",sep="") ,row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.m, paste(visits_less,"visits_meaning_other_masked.csv",sep="") ,row.names = F)
...
suppressPackageStartupMessages(library(DT))
Res.count2.my<-data.table(table_name="VISIT_OCCURRENCE", Results.visit_occurrence$COUNT2_MY)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(visits_dir,"visits_meaning_year_2categories.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.count2.my, paste(visits_less,"visits_meaning_year_2categories_masked.csv",sep=""),row.names = F)
...
suppressPackageStartupMessages(library(DT))
Res.count1.my<-data.table(table_name="VISIT_OCCURRENCE", Results.visit_occurrence$COUNT1_MY)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(visits_dir,"visits_meaning_year_other.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.count1.my, paste(visits_less,"visits_meaning_year_other_masked.csv",sep=""), row.names = F)
```
...
The VISIT_OCCURRENCE table does not contain continous variables.
<br>
...
suppressPackageStartupMessages(library(DT))
Res.dates.m<-data.table(table_name="VISIT_OCCURRENCE", Results.visit_occurrence$DATES.M)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(visits_dir,"visits_meaning_dates.csv",sep=""), row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.m, paste(visits_less,"visits_meaning_dates_masked.csv",sep=""), row.names = F)
```
...
Res.dates.my<-data.table(table_name="VISIT_OCCURRENCE",Results.visit_occurrence$DATES_MY)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(visits_dir,"visits_meaning_year_dates.csv",sep=""),row.names = F)
...
#Save results as .csv file
write.csv(Res.dates.my, paste(visits_less,"visits_meaning_year_dates_masked.csv",sep=""),row.names = F)
```
...
Numerator= count  
Denominator= total   
Year= retrieved from the year part of the variable `visit_start_date`. Exception for the date counts where the year is retrieved from the year part of the date variable itself.

```{r calculations_results, echo=FALSE}
suppressPackageStartupMessages(library(DT))
calculations_visits<-data.table(rbind(
  cbind(calculation= "Counts stratified by meaning(other variables)", variable_name=c("person_id", "visit_occurrence_id", "speciality_of_visit", "status_at_discharge"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
    cbind(calculation= "Counts stratified by meaning and year(other variables)", variable_name=c("person_id", "visit_occurrence_id", "speciality_of_visit", "status_at_discharge"), numerator=c("Number of unique persons", "Number of unique visit occurrence id", "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
      cbind(calculation= "Counts stratified by meaning(2 or more categories)", variable_name=c("speciality_of_visit_vocabulary", "status_at_discharge_vocabulary", "meaning_of_visit", "origin_of_visit"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning")),
      cbind(calculation= "Counts stratified by meaning and year(2 or more categories)", variable_name=c("speciality_of_visit_vocabulary", "status_at_discharge_vocabulary", "meaning_of_visit", "origin_of_visit"), numerator=c("Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category", "Number of complete observations per category"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning", variable_name=c("visit_start_date", "visit_end_date"), numerator=c( "Number of complete observations per meaning category", "Number of complete observations per meaning category"), denominator=c("Number of total observations with a recorded meaning", "Number of total observations with a recorded meaning in a particular year")),
  cbind(calculation= "Date counts stratified by meaning and year", variable_name=c("visit_start_date", "visit_end_date"), numerator=c( "Number of complete observations per meaning category in a particular year", "Number of complete observations per meaning category in a particular year"), denominator=c("Number of total observations with a recorded meaning in a particular year", "Number of total observations with a recorded meaning in a particular year"))
))
datatable(calculations_visits)
```
...
VISIT_OCCURRENCE     
**1.** VISIT_OCCURRENCE.html: Rmarkdown report      
**2.** `visits_meaning_2categories.csv`: counts of variables with 2 or more categories stratified by meaning      
**3.** `visits_meaning_other.csv`: counts of other variables stratified by meaning          
**4.** `visits_meaning_year_2categories.csv`: counts of variables with 2 or more categories stratified by meaning and year         
**5.** `visits_meaning_year_other.csv`: counts of other variables stratified by meaning and year         
**6.** `visits_meaning_dates.csv`: counts of date variables stratified by meaning           
**7.** `visits_meaning_year_other.csv`: counts of date variables stratified by meaning and year    
**8.** Masked:      
&nbsp;&nbsp;&nbsp;&nbsp;***a.*** `visits_meaning_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning where counts smaller than 5 are masked          
&nbsp;&nbsp;&nbsp;&nbsp;***b.*** `visits_meaning_other_masked.csv`: counts of other variables stratified by meaning where counts smaller than 5 are masked       
&nbsp;&nbsp;&nbsp;&nbsp;***c.*** `visits_meaning_year_2categories_masked.csv`: counts of variables with 2 or more categories stratified by meaning and year where counts smaller than 5 are masked            
&nbsp;&nbsp;&nbsp;&nbsp;***d.*** `visits_meaning_year_other_masked.csv`: counts of other variables stratified by meaning and year where counts smaller than 5 are masked     
&nbsp;&nbsp;&nbsp;&nbsp;***e.*** `visits_meaning_dates_masked.csv`: counts of date variables stratified by meaning where counts smaller than 5 are masked               
&nbsp;&nbsp;&nbsp;&nbsp;***f.*** `visits_meaning_year_dates_masked.csv`: counts of date variables stratified by meaning and year where counts smaller than 5 are masked    
